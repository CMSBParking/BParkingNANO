...
void BToKstarLLBuilder::produce(edm::StreamID, edm::Event &evt, edm::EventSetup const &) const {
  ...
  for(size_t kstar_idx = 0; kstar_idx < kstars->size(); ++kstar_idx) {
    ...
    for(size_t ll_idx = 0; ll_idx < dileptons->size(); ++ll_idx) {
      ...
      // assign correct masses on tracks
      ...
      cand.addUserFloat("b0fitted_kstar_mass" ,(fitter.daughter_p4(0) + fitter.daughter_p4(1)).mass());
      cand.addUserFloat("b0fitted_kstar_pt" ,(fitter.daughter_p4(0) + fitter.daughter_p4(1)).pt());
      cand.addUserFloat("b0fitted_kstar_eta" ,(fitter.daughter_p4(0) + fitter.daughter_p4(1)).eta());
      cand.addUserFloat("b0fitted_kstar_phi" ,(fitter.daughter_p4(0) + fitter.daughter_p4(1)).phi());
